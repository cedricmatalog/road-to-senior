  ...
  return (
    <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px' }}>
      ...
      <section style={{ paddingTop: '96px', paddingBottom: '80px', borderBottom: '1px solid var(--border)' }}>
        <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--accent)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '24px' }}>
        ...
        <p style={{ fontFamily: 'var(--sans)', fontSize: '18px', color: 'var(--text-dim)', fontWeight: 300, maxWidth: '480px', marginTop: '28px', lineHeight: '1.7' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', borderBottom: '1px solid var(--border)' }}>
        {[
          ...
          <div key={i} style={{
            ...
            <p style={{ fontFamily: 'var(--mono)', fontSize: '40px', fontWeight: 500, color: 'var(--accent)', lineHeight: 1, margin: 0 }}>{stat.value}</p>
            <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--text-dim)', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: '8px' }}>{stat.label}</p>
          </div>
        ))}
      </section>
      ...
      <section style={{ padding: '64px 0', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: 'var(--border)', borderBottom: '1px solid var(--border)' }}>
        {[
          ...
          <Link key={i} href={track.href} className="track-card" style={{
            ...
            <span style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--accent)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>{track.tag}</span>
            <h2 style={{ fontFamily: 'var(--mono)', fontSize: '22px', fontWeight: 500, color: 'var(--text)', lineHeight: 1.3, margin: '16px 0 20px', whiteSpace: 'pre-line' }}>{track.title}</h2>
            <p style={{ fontFamily: 'var(--sans)', fontSize: '14px', color: 'var(--text-dim)', lineHeight: 1.7, margin: 0 }}>{track.body}</p>
          </Link>
        ))}
      </section>
      ...
      <footer style={{ padding: '24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--text-faint)', letterSpacing: '0.05em', margin: 0 }}>Progress saved locally — no account required</p>
        <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--text-faint)', letterSpacing: '0.05em', margin: 0 }}>road-to-senior</p>
      </footer>
